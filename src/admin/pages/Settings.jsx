...
export default function SettingsPage() {
  ...
  const impactSave = async (event) => {
    ...
    await handleSave(
      ...
      { successMessage: 'Impact section saved' }
    )
  }

  const addSocial = () => setSocials((prev) => [...prev, { id: makeId(), label: '', href: '', icon: '' }])
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-slate-900">Directories</h2>
            <p className="text-xs text-slate-500">Manage locations and gotras on dedicated pages.</p>
            <div className="mt-4 flex flex-wrap gap-3">
              <Link to="/admin/locations" className="px-3 py-2 text-sm border border-slate-300 rounded">
                Open locations
              </Link>
              <Link to="/admin/gotras" className="px-3 py-2 text-sm border border-slate-300 rounded">
                Open gotras
              </Link>
            </div>
          </div>
          ...
          <form onSubmit={socialsSave} className="rounded-xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Displayed in the footer and contact areas.</p>
              </div>
            ...
            <div className="space-y-3">
              {socials.map((social) => (
                <div key={social.id} className="border border-slate-200 rounded-lg p-3 grid md:grid-cols-[1fr_2fr_1fr_auto] gap-2 items-end">
                  ...
                  <button type="button" onClick={() => setSocials((prev) => prev.filter((row) => row.id !== social.id))} className="px-3 py-2 text-sm border border-red-300 text-red-600 rounded">
                    Remove
                  </button>
                </div>
              ))}
          ...
          <form onSubmit={linksSave} className="rounded-xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Shown in footer navigation lists.</p>
              </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <LinkList
                ...
                onRemove={(id) => setLinks((prev) => ({ ...prev, quick: prev.quick.filter((x) => x.id !== id) }))}
              />
              <LinkList
                title="Secondary links"
                links={links.secondary}
                onChange={(link) => setLinks((prev) => ({ ...prev, secondary: updateById(prev.secondary, link.id, link) }))}
                onRemove={(id) => setLinks((prev) => ({ ...prev, secondary: prev.secondary.filter((x) => x.id !== id) }))}
              />
            </div>
          </form>

          <form onSubmit={impactSave} className="rounded-xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Homepage impact</h2>
                <p className="text-xs text-slate-500">Shown on the home page as stats and milestones.</p>
              </div>
  ...
  return (
    <div className="border border-slate-200 rounded-lg p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => onChange({ id: makeId(), label: '', href: '' })} className="px-3 py-2 text-sm border border-slate-300 rounded">
          Add
        </button>
      </div>
      ...
      {links.map((link) => (
        <div key={link.id} className="grid grid-cols-[1fr_2fr_auto] gap-2 items-end">
          <Field label="Label" value={link.label} onChange={(e) => onChange({ ...link, label: e.target.value })} />
          <Field label="URL" value={link.href} onChange={(e) => onChange({ ...link, href: e.target.value })} />
          <button type="button" onClick={() => onRemove(link.id)} className="px-3 py-2 text-sm border border-red-300 text-red-600 rounded h-10">
            Remove
          </button>
        </div>
      ))}
...
function updateById(arr, id, updates) {
  return (Array.isArray(arr) ? arr : []).map((item) => (item.id === id ? { ...item, ...updates } : item))
}
...
const normalizeLinks = (value) => {
  ...
  return {
    quick: Array.isArray(v.quick) ? v.quick.map((l) => ({ id: l.id || makeId(), label: l.label || '', href: l.href || '' })) : [],
    secondary: Array.isArray(v.secondary) ? v.secondary.map((l) => ({ id: l.id || makeId(), label: l.label || '', href: l.href || '' })) : [],
  }
}

const normalizeImpact = (value) => {
  const v = value && typeof value === 'object' ? value : defaultImpact
  return {
    stats: Array.isArray(v.stats) ? v.stats.map((s) => ({ id: s.id || makeId(), ...s })) : [],
    milestones: Array.isArray(v.milestones) ? v.milestones.map((m) => ({ id: m.id || makeId(), ...m })) : [],
  }
}
